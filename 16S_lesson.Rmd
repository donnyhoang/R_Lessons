...
sample.names <- sapply(strsplit(basename(fnFs), "_"), `[`, 1) #remove unnecessary part of filenames
```
...
Here, we estimated error rates. The y-axis is error rate frequency, and the x-axis represents the consensus quality score. The black line is our estimated error rate, and the dots are the observed error rate.The estimated error rate seems like a good fit for our observed error rates. Let's move forward.
...
The dada2 algorithm has inferred that I have 45 "true sequence variants", which are inferred from the 5064 sequences in inputted.
...
The majority of my reads are from Cellvibrio, a known cellulose degrader. I think this finding makes sense, since we sequenced a cellulose degrading community. We will dig into the data we generated more on Wednesday. This is the end of part one. We will pick up with visualizing our data next lesson. For now, save your script and the R Workspace image so all your progress is saved.

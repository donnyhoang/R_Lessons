...
Let's take a quick look at the data. What compounds did we measure?
...
Before we start interpreting the data, I want to make a couple aesthetic changes. First, these colors are awful. Having a gradient of color for categorical variables can make interpretation difficult. Closeness in color can imply closeness in value - which doesn't make sense for discrete variables. However, I don't want to manually type in colors for all 11 of our variables. We can get around this with RColorBrewer, which is a popular color palette package in R. Many of the color palettes are colorblind friendly (though we won't be using one is it doesn't have enough colors for us).

Second, R has reordered all our samples alphabetically, which isn't that useful for presenting our data in this situation. Let's tackle the colors first.
...
This graph can be hard to interpret as is, but you can zoom into the plot on RStudio if you wish. Since this is an exploratory analysis, let's look at each graph individually.
